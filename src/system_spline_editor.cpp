...
namespace {
    void SplineDetails(ComponentSpline& spline) {
        ImGui::PushID(&spline);
...
void SystemSplineEditor::Draw(GameWorld& world, canyon::graphics::IGraphics& graphics) {
    ...
    ImGui::Begin("Splines");
    static std::array<char, 1024> nameBuf;
    ImGui::InputText("Spline name", nameBuf.data(), nameBuf.size());
    if (ImGui::Button("Create")) {
        auto entity = registry.create();
        auto& spline = registry.emplace<ComponentSpline>(entity);
        spline.m_name = nameBuf.data();
    ...
    for (auto [entity, spline] : view.each()) {
        if (ImGui::CollapsingHeader(spline.m_name.c_str())) {
            ...
            SplineDetails(spline);
        }
        if (spline.m_visible && spline.m_spline.size() > 2) {
            DrawSpline(graphics, spline);
        }
    }
    ImGui::End();
}
